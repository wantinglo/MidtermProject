...
<body>
	<div id="wrapper">
		<div id="container">
			...
			<div id="img-wrap">
				<ul class="images">
					<li class="big-img" id="image1"><img id="image"
				...
				<ul class="colors">
					<c:forEach var="color" items="${colorlist}">
						<li role="radio" id="color"  value="${color}"
							onclick="changeimg(this)" />${color}</li>
					</c:forEach>
				</ul>
			</div>
			...
			<div class="info">
				...
				<%-- 加入購物車 --%>
				<input id="cartBtn" type="button" class="button" value="Add to cart">
			</div>
...
<script type="text/javascript">
	...
	function changeimg(obj) {
		$.ajax({
			url : "/HibernateWebProject/productC/changeimg",
			type : "POST",
			dataType : "json",
			data : {
				"p_name" : $("#p_name").text(),
			...
			success : function(res) {
				$("#image").attr('src','<c:url value="/productC/getImage?img='+ res.p_img+ '" />' );
